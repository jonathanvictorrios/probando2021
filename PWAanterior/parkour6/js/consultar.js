...
$(function() {
    ...
    $.get("./control/buscarTitulosImagenes.php", function(data) {
        for (let i = 0; i < 12; i++) {
            console.log(data);
            var objImagen=data[i];
            idImagen = objImagen.id_imagen;
            nombreImagen = objImagen.nombre_imagen;
            //armo el modal
            var modalFade = $('<div></div>');
            modalFade.attr('class', 'modal fade');
            modalFade.attr('id', 'c' + objImagen.id_imagen);
            modalFade.attr('tabindex', '-1');
            modalFade.attr('aria-labelledby', objImagen.nombre_imagen+'Label');
    ...
    if (this.value != "") {
        ...
        $.post("./control/buscarEstados.php", { caracter: this.value }, function(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                sugerencias = objEstado.nombre_pais;
                item.text(sugerencias);
                $("#campoSugerenciasEstado").append(item);

                $('#paisElegido').attr('value',objEstado.id_pais);
            }
        }, "json");
    } else {
        $('#campoSugerenciasEstado').empty();
...
$('#campoProvincia').on('keyup', function() {
    ...
    if (this.value != "") {
        $.post("./control/buscarProvincias.php", { caracterProvincia: this.value,idPaisElegido:idPaisElegido}, function(data) {
            ...
            select.attr('id','selectProvincias');
            select.attr('aria-label' , 'size 3 select example');
            
            select.attr('size',3);

            for (let i = 0; i < data.length; i++) {
                var idProvincia=data[i].id_provincia;
                var option=$('<option></option>');
                option.attr('class','text-dark');
                option.attr('value',idProvincia);
                var objProvincia = data[i];
                sugerencias = objProvincia.nombre_provincia;
                option.text(sugerencias);
                select.append(option);

                $("#campoSugerenciasProvincia").append(select);
            }
...
/*
$("#sugGrup").keyup(function() {
    if (this.value != "") {
        $.post("./control/grupos.php", { caracter: this.value }, function(data) {
            for (let i = 0; i < data.length; i++) {
                var objGrupo = data[i]; //de la coleccion del grupo de pk vamos de I hasta N
                var agregar = $('<option></option>'); //creamos option del select
                agregar.attr('value', objGrupo.id_equipo);
                agregar.attr('class', 'sugerenciaGrupo'); //agregamos el atributo valor
                agregar.text(objGrupo.nombre_equipo); //texto al option
                $('#grupo').append(agregar); //llevamos al html donde se encuentra el id
            }
        }, "json");
